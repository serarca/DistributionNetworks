...
srun python ~/vrp_project/Code/PipelineScripts/resolve_instance.py instances/%s/%s ~/vrp_project/ %s -p normal,dev,gpu
...
for c in clusters:
    for d in range(days_analysis):
        ...
        with open(path, 'a') as file:
            file.write(header % ('00:20',"daily",instance,results_folder_ts,penalty,"daily",instance,results_folder_ts))
...
for c in clusters:
    ...
    open(path, 'w').close()
    with open(path, 'a') as file:
        file.write(header % ('01:30',"temporal",instance,results_folder_ts,penalty,"temporal",instance,results_folder_ts))
    with open(submission, 'a') as file:
        file.write('sbatch ' + cluster_path + instance + '.sh' + '\n')
...
for d in range(days_analysis):
    ...
    open(path, 'w').close()
    with open(path, 'a') as file:
        file.write(header % ('01:30',"spatial",instance,results_folder_ts,penalty,"spatial",instance,results_folder_ts))
    with open(submission, 'a') as file:
        file.write('sbatch ' + cluster_path + instance + '.sh' + '\n')